...
from averageUI import Ui_Dialog
...
class Principal(QtGui.QMainWindow):
    ...
    def on_action_Average_triggered(self):
        self.editDlg = EditPromedio(self)
        actionDlg = self.editDlg.exec_()
        if actionDlg:
            self.minutosAvg = 0
            self.promedios = []
            self.promediosAvg = []
            self.selComboBox = str(self.editDlg.ui_avg.ui_comboTiempo.currentText())
            self.selSpinBox = self.editDlg.ui_avg.ui_spinBox.value()
            if self.selComboBox == "minute(s)" :
                self.timeToAvg = self.selSpinBox
                self.ui.ui_label_2.setText("Average "+str(self.selSpinBox)+" [min]")
            else:
                self.timeToAvg = self.selSpinBox * 60
                self.ui.ui_label_2.setText("Average "+str(self.selSpinBox)+" [hour]")
...
#            self.valores =  concatenate((self.valores[1:],[self.termperatura]),1)
        if self.tiempo_maximo > self.time : # Si los datos no superan los 60 datos.
            ...
            if self.time % 60 == 0 :
                ...
                self.minutos += 1
                self.minutosAvg += 1
                self.ui.ui_lcdTime.display(self.minutos) 
                self.fichero.write(str(self.promedio)+"\n")
                #print "minutos",self.minutos # Debug
                print "min Promedio",self.minutosAvg #Debug 
                print "valores promedios",self.promediosAvg
                if self.minutosAvg == self.timeToAvg : # 
                    self.minutosAvg = 0
                    ...
                    self.promediosAvg.append(self.promedioMints)
                    self.fichero2.write(str(self.promedioMints)+"\n")
...
# Average
#------------------------------------------------------------------------------
# Crear una clase para nuestra ventana de Configurar promedio
class EditPromedio(QtGui.QDialog):
    def __init__ (self, parent):
        QtGui.QDialog.__init__(self, parent)
        self.ui_avg=Ui_Dialog()
        self.ui_avg.setupUi(self)
